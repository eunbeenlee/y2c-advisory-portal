...
async function fetchItems() {
  ...
  try {
    ...
    if (result.success) {
      ...
      document.getElementById('orderTaxLabel').innerText = `Estimated Tax - ${taxRateObj.name}:`;

      const regionInfo = currentClientState === "DEFAULT" ? "Standard" : currentClientState;
...
function renderTableItems() {
  ...
  if (sLabel) {
    sLabel.innerText = userRole === "MASTER" ? (masterViewRegion === "ALL" ? "Canada Total Stock" : `Hub Stock (${masterViewRegion})`) : `Local Hub (${currentClientState})`;
  }
  ...
  cachedItems.forEach((item, index) => {
    ...
    let displayStock = userRole === "MASTER" ? (masterViewRegion === "ALL" ? item.totalStock : (item.stockBreakdown[masterViewRegion] || 0)) : item.regionalStock;
    ...
    if (isStockEditMode && userRole === "MASTER") {
      ...
      for (const reg in item.stockBreakdown) {
        editInputs += `<div class="flex items-center justify-between gap-2 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100 mb-1"><span class="text-[9px] font-black text-emerald-800">${reg}</span><input type="number" min="0" data-code="${item.code}" data-region="${reg}" value="${item.stockBreakdown[reg]}" class="stock-region-input w-12 bg-white border border-emerald-400 rounded px-1 text-center text-[11px] font-bold focus:outline-none"></div>`;
      }
...
function calculateOrderTotal() {
  ...
  qtyInputs.forEach(input => {
    ...
    if (qty > maxQty) {
      input.value = maxQty; 
      showToast("재고 수량을 초과하여 주문할 수 없습니다.", "error");
      return;
    }
    ...
    if (qty > 0) {
      ...
      if (cachedItems[idx]) {
        subtotal += (qty * Number(cachedItems[idx].price));
      }
    }
  });
...
async function toggleStockEditMode() {
  ...
  if (!isStockEditMode) {
    ...
    try {
    ...
    } catch (err) { showToast("재고 업데이트 실패: " + err.message, "error"); } 
    finally {
      ...
      if (filter) filter.disabled = false; if (orderContainer) orderContainer.classList.remove('hidden');
    }
  }
}
...
async function submitOrder() {
  ...
  if (orderItems.length === 0) { showToast("발주할 수량을 입력해주세요.", "error"); return; }
...
window.submitOrder = submitOrder; window.fetchItems = fetchItems; window.toggleStockEditMode = toggleStockEditMode; window.applyRegionFilter = applyRegionFilter; window.calculateOrderTotal = calculateOrderTotal;
