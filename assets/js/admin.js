...
if (!sessionToken || userRole !== "MASTER") { window.location.href = "index.html"; }
...
async function saveClientData(rowIdx) {
  ...
  try {
    ...
    } else { showToast("저장 실패: " + result.message, "error"); }
  } catch (err) { showToast("서버 통신 중 오류가 발생했습니다.", "error"); } 
  finally { if (saveBtn && saveBtn.innerText !== "✅ SAVED") { saveBtn.disabled = false; saveBtn.innerText = "SAVE"; saveBtn.classList.remove('animate-pulse'); } }
}
...
function populateSalesClientSelector() {
  ...
  if (cachedClients.length === 0) { clientSelect.innerHTML = `<option value="">가맹점을 불러올 수 없습니다</option>`; return; }
  cachedClients.forEach(c => {
    const opt = document.createElement('option');
...
function recalcSalesRow(month) {
  ...
  const p = parseFloat(posInput.value) || 0, d = parseFloat(delInput.value) || 0, tot = p + d;
...
async function saveSalesGridData() {
  ...
  for (let m = 1; m <= 12; m++) {
    const pos = parseFloat(document.querySelector(`.sales-input-pos[data-month="${m}"]`)?.value) || 0;
    const del = parseFloat(document.querySelector(`.sales-input-del[data-month="${m}"]`)?.value) || 0;
    recordsToSave.push({ month: m, pos: pos, delivery: del });
  }

  const originalHtml = btn.innerHTML;
  btn.disabled = true; btn.innerHTML = "<span>⏳</span> SYNCHRONIZING..."; btn.classList.add('animate-pulse');
...
window.switchAdminTab = switchAdminTab; window.saveClientData = saveClientData; window.loadSalesGrid = loadSalesGrid; window.recalcSalesRow = recalcSalesRow; window.saveSalesGridData = saveSalesGridData;
document.addEventListener('DOMContentLoaded', () => { fetchMasterData(); populateSalesYearSelector(); });
